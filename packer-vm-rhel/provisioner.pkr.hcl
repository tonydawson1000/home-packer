locals {
  ...
  //   hostname = var.hostname == "" ? "rhel_${local.version}" : var.hostname

  //   boot_command = length(var.boot_command) == 0 ? [
  //     "<wait>e<wait><down><down><down><end><wait> autoinstall ds=nocloud-net\\;s=http://{{.HTTPIP}}:{{.HTTPPort}}/ubu *** RHEL *** ntu/<f10><wait>"
  //   ] : var.boot_command

  # Remotely Hosted ISO and Checksum
  # isos_url = var.iso_url == "" ? "https://cdimage.ubu *** RHEL *** ntu.com/releases/${var.version}/release/ubu *** RHEL *** ntu-${var.version}-live-server-arm64.iso" : var.iso_url
  # iso_checksum = var.iso_checksum == "" ? "file:https://cdimage.ubu *** RHEL *** ntu.com/releases/${var.version}/release/SHA256SUMS" : var.iso_checksum

  # Locally Hosted ISO and Checksum
  isos_url     = var.iso_url == "" ? "iso/${var.version}/rhel-${var.version}-aarch64-boot.iso" : var.iso_url
  ...
  machine_name = var.machine_name == "" ? "rhel-${local.version}" : var.machine_name
